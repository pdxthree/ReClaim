...
def compute_rouge(data):
    ...
    for idx, item in enumerate(data):
        ...
        if "annotations" in item and item['annotations'] is not None and item['annotations'] != []: 
...
def compute_qa(data):
    ...
    qa_pipeline = pipeline("question-answering", model=QA_MODEL, device=6)
    logger.info("Done")
...
def _run_nli_autoais(passage, claim):
    ...
    input_ids = autoais_tokenizer(input_text, return_tensors="pt").input_ids.to("cuda:6")
...
def compute_claims(data):
    ...
    if autoais_model is None:
        ...
        autoais_model = AutoModelForSeq2SeqLM.from_pretrained(AUTOAIS_MODEL, torch_dtype=torch.bfloat16, max_memory=get_max_memory(), device_map="cuda:6")
...
def compute_autoais(data,
    ...
    if sent_mcite > 0 and sent_mcite_support > 0:
        print("Among all sentences, %.2f%% have no citation, %.2f%% have multiple citations, among which %.2f%% are supported by the joint set, among which %.2f%% overcite." % (
            100 * no_citations / sent_total,
...
def main():
    ...
    for i in range(len(data)):
        data[i]['output'] = data[i]['reranker_answer'].strip()
